...
def _logo_image(logo_b64: str | None, width_mm: float = 28, height_mm: float = 28):
    if not logo_b64:
        return None
    try:
...
def _header(org: dict, styles):
    logo = _logo_image(org.get("logo_base64"))
    right_text = f"""
...
def _footer_text(org: dict) -> str:
    ...
    return " | ".join([p for p in parts if p and p.strip() and p != "C.F. " and p != "PEC: "])
...
def generate_receipt_pdf(receipt: dict, tesserato: dict, org: dict,
    ...
    """Generate a receipt PDF matching the Wolf's Mind ASD layout."""
    buf = BytesIO()
    ...
    story.append(Paragraph("RICEVUTA", st["title"]))
    num_row = Table([[
        Paragraph(f"<b>N. {receipt['numero']}</b>", ParagraphStyle(
            "n", fontSize=12, textColor=TEXT, fontName="Helvetica-Bold")),
        Paragraph(f"<b>Data:</b> {_fmt_date(receipt['data'])}", ParagraphStyle(
            "d", fontSize=11, alignment=TA_RIGHT, textColor=TEXT)),
    ]], colWidths=[90 * mm, 90 * mm])
    num_row.setStyle(TableStyle([("VALIGN", (0, 0), (-1, -1), "MIDDLE")]))
    story.append(num_row)
    story.append(Spacer(1, 6 * mm))

    # DATI TESSERATO section
    story.append(_section_bar("DATI DEL TESSERATO / RICEVENTE", st))
    dt_data = [
        [Paragraph("Cognome e Nome:", st["label"]),
    ...
    t1 = Table(dt_data, colWidths=[30 * mm, 65 * mm, 30 * mm, 55 * mm])
    t1.setStyle(_field_style())
    story.append(t1)

    addr_data = [[
        Paragraph("Indirizzo:", st["label"]),
    ...
    t2 = Table(addr_data, colWidths=[22 * mm, 55 * mm, 12 * mm, 20 * mm, 15 * mm, 56 * mm])
    ...
    pay_rows = [[
        ...
        Paragraph("<b>N. lezioni</b>", ParagraphStyle("h", fontSize=10, alignment=TA_CENTER, fontName="Helvetica-Bold")),
        Paragraph("<b>Importo totale</b>", ParagraphStyle("h2", fontSize=10, alignment=TA_RIGHT, fontName="Helvetica-Bold")),
    ]]
    for item in receipt.get("items", []):
        pay_rows.append([
            Paragraph(item.get("descrizione", ""), st["val"]),
    ...
    pay = Table(pay_rows, colWidths=[100 * mm, 30 * mm, 50 * mm])
    ...
    # Totale + metodo pagamento
    tot_data = [[
        Paragraph("Metodo di pagamento:", st["label"]),
        Paragraph(receipt.get("metodo_pagamento", "Contanti"), st["val"]),
        Paragraph("<b>Totale ricevuto:</b>", ParagraphStyle(
            "tl", fontSize=10, alignment=TA_RIGHT, fontName="Helvetica-Bold")),
        Paragraph(f"<b>{_fmt_eur(receipt.get('totale', 0))}</b>", ParagraphStyle(
            "tv", fontSize=12, alignment=TA_RIGHT, fontName="Helvetica-Bold", textColor=PRIMARY)),
    ]]
    tot = Table(tot_data, colWidths=[45 * mm, 55 * mm, 40 * mm, 40 * mm])
    ...
    fs = Table(firma_data, colWidths=[90 * mm, 90 * mm])
...
def generate_balance_report_pdf(org: dict, movimenti: list, date_from: str,
    ...
    """Generate a balance report PDF (entrate/uscite/saldo)."""
    buf = BytesIO()
    ...
    story = [_header(org, st), Spacer(1, 8 * mm),
             ...
             Paragraph(f"Periodo: <b>{_fmt_date(date_from)}</b> — <b>{_fmt_date(date_to)}</b>",
                       ParagraphStyle("p", fontSize=11, alignment=TA_CENTER, textColor=TEXT)),
             Spacer(1, 6 * mm)]

    # Riepilogo
    story.append(_section_bar("RIEPILOGO", st))
    riep = [[
        Paragraph("<b>Entrate</b>", st["val"]),
    ...
    rows = [[
        ...
        Paragraph("<b>Descrizione</b>", st["val"]),
        Paragraph("<b>Importo</b>", ParagraphStyle("h", fontSize=10, alignment=TA_RIGHT,
                                                    fontName="Helvetica-Bold")),
    ]]
    for m in movimenti:
        ...
        rows.append([
            Paragraph(_fmt_date(m["data"]), st["val"]),
            Paragraph(m["tipo"].capitalize(), st["val"]),
            Paragraph(m.get("categoria", ""), st["val"]),
            Paragraph(m.get("descrizione", "")[:60], st["val"]),
