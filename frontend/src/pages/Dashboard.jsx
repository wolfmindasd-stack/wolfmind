    ...
    <div className="space-y-8" data-testid="dashboard-page">
      <div>
        ...
        <p className="text-white/50 mt-2 text-sm">
          {isTecnico
            ? "I tuoi dati: tesserati, ricevute, movimenti e compenso maturato."
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <KpiCard
          title={isTecnico ? "Tuoi tesserati" : "Tesserati totali"}
          value={d?.tesserati_count ?? 0}
          icon={Users}
          testid="kpi-tesserati"
        />

        <KpiCard
          title={isTecnico ? "Tuoi abbonamenti" : "Abbonamenti"}
          value={d?.abbon_count ?? 0}
          icon={Package}
          testid="kpi-abbonamenti"
        />

        <KpiCard
          title="Ricevute del mese"
          value={d?.ricevute_mese_count ?? 0}
          icon={Receipt}
          hint={fmtEur(d?.incassato_mese ?? 0) + " incassati"}
          testid="kpi-ricevute-mese"
        />

        {isTecnico ? (
          <KpiCard
            title="Compenso maturato"
            value={fmtEur(d?.compenso_maturato ?? 0)}
            icon={Wallet}
            color="text-[#FFCC00]"
            testid="kpi-compenso"
            hint="Anno in corso"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="wm-card p-6 lg:col-span-1">
          ...
          <div
            className={`font-display text-4xl sm:text-5xl font-black tracking-tighter ${
              (d?.saldo_anno ?? 0) >= 0 ? "text-[#34C759]" : "text-[#FF3B30]"
            }`}
            data-testid="kpi-saldo"
          >
            {fmtEur(d?.saldo_anno ?? 0)}
          </div>

          <div className="mt-4 text-sm text-white/50">
            {isTecnico
              ? "Entrate meno uscite attribuite a te (anno in corso)."
          ...
          {!isTecnico && (
            <div className="mt-4 pt-4 border-t border-white/10 space-y-1">
              <div className="flex justify-between text-sm">
                <span className="text-white/50">Entrate</span>
                <span className="text-[#34C759] font-semibold">{fmtEur(d?.entrate_anno ?? 0)}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-white/50">Uscite</span>
                <span className="text-[#FF3B30] font-semibold">{fmtEur(d?.uscite_anno ?? 0)}</span>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="wm-card p-6 lg:col-span-2" data-testid="scadenze-panel">
          ...
          <div className="space-y-2">
            {Array.isArray(d?.scadenze_imminenti) &&
